...
# ============================================
# Step 7: Configure ALSA with File Plugin (tee to FIFO)
# ============================================
echo "Configuring ALSA for $HAT_NAME..."

# Create FIFO directory and pipe
echo "Setting up audio FIFO for spectrum analyzer..."
mkdir -p /tmp/audio
if [ ! -p /tmp/audio/stream.fifo ]; then
    mkfifo /tmp/audio/stream.fifo
fi
chmod 666 /tmp/audio/stream.fifo

# Add tmpfs mount for /tmp/audio to persist across reboots
if ! grep -q "/tmp/audio" /etc/fstab 2>/dev/null; then
    echo "tmpfs /tmp/audio tmpfs defaults,size=2M,mode=1777 0 0" >> /etc/fstab
fi

# Generate ALSA config with file plugin (tee to FIFO for spectrum analyzer)
# The file plugin writes to the DAC first, then tees PCM to the FIFO.
# If the FIFO is not readable, audio continues playing normally.
cat > /etc/asound.conf << EOF
# ALSA configuration for $HAT_NAME with spectrum analyzer
# Generated by setup script
# Audio is tee'd to FIFO for real-time spectrum analysis
...
pcm.tee_dac {
    type file
    slave.pcm "hw:$HAT_CARD_NAME,0"
    file /tmp/audio/stream.fifo
    format "raw"
}
...
echo "  - Audio tee to FIFO enabled for spectrum analyzer"
echo ""
