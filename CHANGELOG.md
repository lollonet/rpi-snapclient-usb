...
### Added
- **Network-Aware ALSA Tuning** ([#74](https://github.com/lollonet/rpi-snapclient-usb/pull/74)) - Auto-detect WiFi vs Ethernet and set appropriate ALSA buffer defaults (WiFi: 250ms/8 frags, Ethernet: 150ms/4 frags); overridable via `snapclient.conf`
- **WiFi Power Save Disabled** ([#74](https://github.com/lollonet/rpi-snapclient-usb/pull/74)) - Automatically disable WiFi power management during setup to prevent audio dropouts; persistent via NetworkManager config
- **CONNECTION_TYPE Config** ([#74](https://github.com/lollonet/rpi-snapclient-usb/pull/74)) - New `.env` variable for network type; auto-detected or manually overridable

### Changed
- **Snapclient URL Format** ([#74](https://github.com/lollonet/rpi-snapclient-usb/pull/74)) - Migrate entrypoint.sh from deprecated `--host`/`--port` flags to `tcp://host:port` URL format
